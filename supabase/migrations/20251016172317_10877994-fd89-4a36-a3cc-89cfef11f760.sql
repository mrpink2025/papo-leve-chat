-- Fix get_user_conversations RPC function with proper column qualification
CREATE OR REPLACE FUNCTION public.get_user_conversations(p_user_id uuid, p_include_archived boolean DEFAULT false)
 RETURNS TABLE(conversation_id uuid, conversation_type text, conversation_name text, conversation_avatar_url text, conversation_updated_at timestamp with time zone, archived boolean, last_message_content text, last_message_created_at timestamp with time zone, unread_count bigint, member_count bigint, other_user_id uuid, other_username text, other_full_name text, other_avatar_url text, other_status text, other_bio text)
 LANGUAGE plpgsql
